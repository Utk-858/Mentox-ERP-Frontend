...
const Sidebar: React.FC<SidebarProps> = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  const [activeItem, setActiveItem] = React.useState(currentItem?.label || "Lectures");
  ...
  return (
    ...
    >
      <div onClick={toggleSidebar} className="flex items-center gap-4 cursor-pointer mb-6">
        <div className={`flex flex-row items-center ${sidebarOpen ? "ml-7 mt-1 gap-3" : "gap-0"}`}>
      ...
      {sidebarOpen ? (
        <nav className="ml-6 flex flex-col mt-4 gap-3.5 text-[15px] overflow-y-auto">
          {navItems.map((item) => (
            <NavLink to={item.path} key={item.label} onClick={() => setActiveItem(item.label)}>
              <NavItem icon={item.icon} label={item.label} active={activeItem === item.label} />
            </NavLink>
          ))}
        </nav>
      ) : (
        <nav className="flex flex-col justify-between items-center h-full overflow-y-auto">
          ...
          <div className="mt-20">
            <NavLink to="/SignOut" onClick={() => setActiveItem("Sign Out")}>
              <button
                className={`${
                  activeItem === "Sign Out" ? "bg-white/20" : "hover:bg-white/10"
...
const NavItem = ({ icon, label, active }: any) => (
  <div
    className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer ${
      active ? "bg-[#702DFF] text-white font-semibold" : "text-gray-600 hover:bg-gray-100"
    }`}
  >
    {icon}
    <span>{label}</span>
