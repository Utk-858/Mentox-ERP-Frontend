...
export const TicketDetailView: React.FC<{
  ...
  const handleStatusChange = (newStatus: string) => {
    ...
    onUpdate({ ...ticket, status: newStatus, remarks: ticket.remarks });
    onBack();
  };
  ...
  const handleAddRemark = () => {
    ...
    onUpdate({ ...ticket, status, remarks: updatedRemarks });
    setNewRemark("");
    onBack();
  };
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-6xl mx-auto my-8">
      {/* Header Section */}
      ...
      <div className="mb-4 space-y-3">
        {(ticket.remarks && ticket.remarks.length > 0) ? (
          ticket.remarks.map((remark) => (
            <div key={remark.id} className="bg-gray-100 rounded p-3">
              <div className="flex justify-between items-center">
                <span className="font-medium">{remark.author}</span>
                <span className="text-xs text-gray-400">{remark.timestamp}</span>
              </div>
              <div className="mt-1 text-gray-800">{remark.text}</div>
            </div>
          ))
        ) : (
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={onBack}
          className="bg-[#702DFF] text-white px-4 py-2 rounded ml-2 cursor-pointer"
        >
          Back
        </button>
      </div>
