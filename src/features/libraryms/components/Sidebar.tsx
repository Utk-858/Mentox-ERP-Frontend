...
const Sidebar = () => {
  ...
  // Determine current route label from URL
  const currentPath = location.pathname.slice(1).toLowerCase();
  const activeFromRoute = navItems.find(item =>
    item.label.toLowerCase().replace(/\s+/g, '-') === currentPath
  )?.label;

  const [activeItem, setActiveItem] = useState(activeFromRoute || "Dashboard");
    ...
    <div className="flex flex-col">
      ...
      >
        ...
        >
          ...
          >
            <img src="https://res.cloudinary.com/dikylfimn/image/upload/v1749105575/dummy_g222rq.png" alt="logo" className="w-8 h-8" />
            {sidebarOpen && (
        ...
        {sidebarOpen ? (
          <nav className="ml-6 flex flex-col gap-3 overflow-y-auto">
            {navItems.map((item) => (
              <NavLink to={`/${item.label.toLowerCase().replace(/\s+/g, '-')}`} key={item.label}>
                <NavItem
                  icon={item.icon}
                  label={item.label}
                  active={activeFromRoute === item.label}
      ...
      {sidebarOpen && (
        <div className="w-64 mt-7 p-4 bg-[#702DFF] rounded-xl text-white text-center space-y-2 mx-1 flex flex-col items-center">
          <img src="https://res.cloudinary.com/dikylfimn/image/upload/v1749105575/dummy_g222rq.png" alt="promo" />
