...
const navItems = [
  { icon: <Home size={16} />, label: "Application", path: "/gate/dashboard" },
  { icon: <BookOpen size={16} />, label: "Visit School", path: "/Courses" },
  { icon: <ClipboardList size={16} />, label: "About School", path: "/To-Do" },
  { icon: <FileText size={16} />, label: "Customer Support", path: "/Quiz" },
{ icon: <FileText size={16} />, label: "Visitor Form", path: "/gate/visitor" },
];
...
const SidebarGatePass: React.FC<SidebarStudentProps> = ({ activeLabel }) => {
        ...
        {sidebarOpen ? (
          ...
          <nav className="flex flex-col justify-between items-center h-full overflow-y-auto">
            <div className="flex flex-col gap-6 mt-4">
              {navItems.slice(0, navItems.length).map((item) => (
                <NavLink
                  to={item.path}
                  key={item.label}
                  onClick={() => setActiveItem(item.label)}
...
export default SidebarGatePass;